...
def initCube(gmm, w=0, rng=np.random) -> None:
...
def initToFillCube(gmm, omega=0.5, rng=np.random) -> None:
    gmm.amp[k] = 1.0 / gmm.K
    ...
    s = (omega * vol_data / gmm.K * gamma(gmm.D * 0.5 + 1)) ** (1.0 / gmm.D) / np.sqrt(np.pi)
...
def insideComponent(k, gmm, coords, covar=None, cutoff=5.0):
    if gmm.amp[k] * K > 0.01:
        return gmm.logL_k(k, coords, covar=covar, chi2_only=True) < cutoff
    return np.zeros(len(coords), dtype="bool")


def GMMSel(coords, gmm, covar=None, sel_gmm=None, cutoff_nd=3.0, rng=np.random):
...
def max_posterior(gmm, U, coords, covar=None):
    ...
    k = 0
    for log_p[k], U[k], _ in parmap.starmap(pygmmis._Estep, zip(range(gmm.K), U, strict=False), gmm, data, covar, None, pm_pool=pool, pm_chunksize=chunksize):
...
if __name__ == "__main__":
    ...
    for _r in range(R):
        ...
        for k in range(K_):
            ...
            assoc_frac[k + counter] = sel__.sum() * 1.0 / nbh0[k].size
