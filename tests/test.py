...
def plotResults(orig, data, gmm, patch=None, description=None, disp=None) -> None:
    ...
    if description is not None:
        ax.text(0.05, 0.95, rf"{description}", ha="left", va="top", transform=ax.transAxes, fontsize=20)
        ax.text(0.05, 0.89, rf"$\log{{\mathcal{{L}}}} = {logL:.3f}$", ha="left", va="top", transform=ax.transAxes, fontsize=20)
    else:
        ax.text(0.05, 0.95, rf"$\log{{\mathcal{{L}}}} = {logL:.3f}$", ha="left", va="top", transform=ax.transAxes, fontsize=20)
...
def plotDifferences(orig, data, gmms, avg, l, patch=None) -> None:
    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplot(111, aspect="equal")
...
def getSelection(type="hole", rng=np.random):
    ...
    if type == "boxWithHole":
        cb = getBoxWithHole
        ps = [patches.Circle([6.5, 6.0], radius=2, fc="none", ec="k", lw=1, ls="dashed"), patches.Rectangle([0, 0], 10, 10, fc="none", ec="k", lw=1, ls="dashed")]
    if type == "cut":
        cb = getCut
...
if __name__ == "__main__":
    ...
    gmm.amp /= gmm.amp.sum()
    gmm.mean[:, :] = np.array([[0.08016886, 0.21300697], [0.70306351, 0.6709532], [0.01087670, 0.852077]]) * 10
    gmm.covar[:, :, :] = np.array([[[0.08530014, -0.00314178], [-0.00314178, 0.00541106]], [[0.03053402, 0.0125736], [0.0125736, 0.01075791]], [[0.00258605, 0.00409287], [0.00409287, 0.01065186]]]) * 100
    ...
    avg = pygmmis.stack(gmms, l)
    print("execution time %ds" % (datetime.datetime.now() - start).seconds)
    plotResults(orig, data, avg, patch=ps, description=r"$\mathtt{GMMis}$")
    ...
    avg = pygmmis.stack(gmms, l)
    print("execution time %ds" % (datetime.datetime.now() - start).seconds)
    plotResults(orig, data, avg, patch=ps, description=r"$\mathtt{GMMis}$ & noise deconvolution")
